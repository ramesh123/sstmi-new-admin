...
const ServiceTypeahead: React.FC<ServiceTypeaheadProps> = ({ onSelect, className = '' }) => {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      setLoading(true);
      try {
        // const response = await fetch('/data/unique_service_types.json', { credentials: 'include' });
        const response = await fetch('/data/unique_service_types_manual.json', { credentials: 'include' });
        if (!response.ok) {
